...
class topic_search
{
	...
	protected function search_with_backend(array $parole, array $vietati, int $limit): ?array
	{
		...
		try
		{
			$backend = $this->resolve_backend($tipo);
	...
	/**
	 * Ottiene il backend di ricerca configurato, provando piu' strade.
	 *
	 * phpBB non salva in search_type l'identificativo di un servizio ma il NOME
	 * DELLA CLASSE con la barra iniziale, per esempio
	 * "\phpbb\search\fulltext_native" oppure, con un motore di terze parti,
	 * "\vendor\estensione\search\backend". Cercarlo tale e quale nel
	 * contenitore non funziona mai: gli identificativi dei servizi non
	 * cominciano con una barra.
	 *
	 * Dalla 3.3.11 phpBB espone una fabbrica che restituisce il backend attivo
	 * senza doverlo cercare: e' la strada piu' pulita e si prova per prima.
	 * Sulle versioni precedenti si ripiega sul contenitore, normalizzando il
	 * nome, e infine sui candidati piu' probabili dell'identificativo.
	 *
	 * @return object|null
	 */
	protected function resolve_backend(string $tipo)
	{
		// 1. La fabbrica dei backend (phpBB 3.3.11 e successivi).
		try
		{
			if ($this->container->has('search.backend_factory'))
			{
				$factory = $this->container->get('search.backend_factory');

				if (method_exists($factory, 'get_active'))
				{
					$attivo = $factory->get_active();
		...
		{
			// Si passa alla strada successiva.
		}

		// 2. Il contenitore, con il nome normalizzato e le forme piu' comuni
		//    dell'identificativo derivato dalla classe.
		$nudo = ltrim($tipo, '\\');

		$candidati = [
			$tipo,
			$nudo,
			// \vendor\estensione\search\backend  ->  vendor.estensione.search.backend
			str_replace('\\', '.', $nudo),
		];

		foreach (array_unique($candidati) as $id)
		{
			if ($id === '')
			{
				continue;
