...
class context_builder
{
	...
	public function build(job $j, bot $b): array
	{
		...
		return [
			'messages' => $messages,
			'stats'    => $stats,
			// Serve al worker per menzionare l'autore: il modello scrive il
			// nome, ma lo user_id lo conosce solo chi ha fatto la query.
			'poster'   => [
				'user_id'  => $j->poster_id,
				'username' => $this->get_poster_name($j->poster_id),
			],
		];
	}
