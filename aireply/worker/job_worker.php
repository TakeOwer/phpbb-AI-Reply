...
class job_worker
{
	...
	public function execute(job $j): string
	{
		...
		$request->system_prompt = $this->build_system_prompt($b, $j);
		...
		$message = $this->format_reply($b, $result->text, $built['poster'] ?? []);
	 ...
	 */
	protected function build_system_prompt(bot $b, job $j): string
	{
		$parts = [];
		...
		/*
		 * La descrizione della board va prima delle istruzioni tecniche e dopo
		 * la personalità: è contesto, non comportamento. Le sezioni elencate
		 * sono quelle leggibili da chi ha scritto il messaggio, mai quelle
		 * visibili al bot — vedi la nota in testa a board_context.
		 */
		if ($b->board_context)
		{
			$contesto = $this->board->build($j->poster_id, $j->forum_id);
		...
		/*
		 * Discussioni collegate, cercate nell'indice della board.
		 *
		 * Indipendente dal contesto della board: si accende da sola nelle
		 * impostazioni generali ed è spenta di serie. Se la ricerca non trova
		 * nulla o il backend non risponde, restituisce una stringa vuota e il
		 * prompt resta quello di prima — il bot risponde comunque.
		 */
		$collegate = $this->search->build((string) $j->request, $j->poster_id, $j->topic_id);
	 ...
	 */
	protected function format_reply(bot $b, string $text, array $poster = []): string
	{
		$body = $this->markdown->convert($text);
		$body = $this->extractor->truncate($body, $b->max_post_chars);

		/*
		 * La menzione si applica al corpo, prima del template.
		 *
		 * Va fatto qui e non sull'intero messaggio: il template contiene la
		 * dichiarazione di contenuto generato, e se per caso vi comparisse il
		 * nome dell'autore verrebbe trasformato anche quello.
		 */
		if ($b->mention_poster && !empty($poster['username']))
		{
			$body = $this->mentions->linkify($body, (int) $poster['user_id'], (string) $poster['username']);
		}
