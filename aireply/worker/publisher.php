...
class publisher
{
	...
	public function __construct(driver_interface $db, user $user, auth $auth, language $language, mention_formatter $mentions, string $root_path, string $php_ext)
	{
		$this->db = $db;
	...
	public function publish(job $j, bot $b, string $message): int
	{
		...
		{
			/*
			 * L'ORDINE DI QUESTE TRE COSE È IL PUNTO DELICATO DEL METODO.
			 *
			 * 1. si assume l'identità del bot
			 * 2. si finge di essere sulla pagina di scrittura
			 * 3. SOLO ALLORA si converte il testo e si pubblica
			 *
			 * La conversione del BBCode va fatta per ultima perché è lì che
			 * Simple mentions decide se interpretare o meno i propri tag, e per
			 * deciderlo guarda due cose: che la pagina corrente sia posting.php
			 * e che chi scrive abbia il permesso `u_can_mention`.
			 *
			 * Convertire prima del cambio utente significa che l'ACL in vigore è
			 * quella del processo cron, che quel permesso non ha. Convertire
			 * fuori da with_posting_page() significa che la pagina non è
			 * posting.php. In entrambi i casi il tag resta testo letterale e
			 * nel post compare "[smention u=2]Nome[/smention]" in chiaro.
			 *
			 * Serve che entrambe le condizioni siano vere nel momento esatto in
			 * cui il parser gira.
			 */
			$this->switch_to($b->user_id);

			$data = $this->mentions->with_posting_page(function () use ($j, $b, $message, $subject, $topic) {
				...
				/*
				 * generate_text_for_storage() non sempre valorizza $uid: succede
				 * quando il testo non contiene BBCode da codificare. Senza uid
				 * il post si salva ma il BBCode non viene interpretato in
				 * visualizzazione.
				 */
				if ($uid === '')
				{
		...
		{
			// Il ripristino deve avvenire anche se qualcosa esplode: lasciare
			// la sessione dell'utente con l'identità del bot sarebbe il tipo di
			// bug che non si scopre subito e fa danni seri.
