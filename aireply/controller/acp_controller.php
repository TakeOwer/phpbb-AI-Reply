...
class acp_controller
{
	...
	public function mode_forums(): void
	{
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			...
			// Solo i forum in cui si può scrivere: categorie e collegamenti
			// non possono ricevere post, quindi non ha senso configurarli.
			if ((int) $row['forum_type'] !== FORUM_POST)
			{
				$this->template->assign_block_vars('forums', [
					...
					'FORUM_ID'    => $forum_id,
					'NAME'        => $row['forum_name'],
					'INDENT'      => $depth[$forum_id],
				]);
				continue;
			}

			$forum_count++;
			$binding = $bindings[$forum_id] ?? null;
			...
			$this->template->assign_block_vars('forums', [
				...
				'NAME'         => $row['forum_name'],
				'INDENT'       => $depth[$forum_id],
				'BOT_ID'       => $binding ? (int) $binding['bot_id'] : 0,
				'ON_NEW_TOPIC' => $binding ? (bool) $binding['on_new_topic'] : true,
				'ON_REPLY'     => $binding ? (bool) $binding['on_reply'] : false,
				'ON_MENTION'   => $binding ? (bool) $binding['on_mention'] : true,
				'DELAY'        => $binding ? (int) $binding['delay_seconds'] : 20,
				'DAILY_CAP'    => $binding ? (int) $binding['daily_cap'] : 30,
				'COOLDOWN'     => $binding ? (int) $binding['cooldown_seconds'] : 120,
				'ENABLED'      => $binding ? (bool) $binding['enabled'] : false,
			]);

			foreach ($bots as $b)
			{
				$this->template->assign_block_vars('forums.bots', [
					'ID'       => $b['bot_id'],
					'NAME'     => $b['label'],
					'SELECTED' => $binding && (int) $binding['bot_id'] === (int) $b['bot_id'],
				]);
			}
		}
	...
	protected function bulk_assign(): array
	{
		...
		// Zero significa rimuovere l'assegnazione: è una scelta esplicita.
		if ($bot_id === 0)
		{
		...
		try
		{
			// Solo i forum selezionati: gli altri conservano la loro configurazione.
			$sql = 'DELETE FROM ' . $this->forums_table . '
				WHERE ' . $this->db->sql_in_set('forum_id', $forum_ids);
			$this->db->sql_query($sql);
	...
	/**
	 * @return array[] indicizzati per forum_id
	 */
	protected function load_bindings(): array
	{
		$result = $this->db->sql_query('SELECT * FROM ' . $this->forums_table);

		$bindings = [];

		while ($row = $this->db->sql_fetchrow($result))
		{
			$bindings[(int) $row['forum_id']] = $row;
		}
	...
	protected function save_forums(): string
	{
		...
		$selected = $this->request->variable('bot', [0 => 0]);
		$on_topic = $this->request->variable('on_new_topic', [0 => 0]);
		$on_reply = $this->request->variable('on_reply', [0 => 0]);
		$on_mention = $this->request->variable('on_mention', [0 => 0]);
		$delay = $this->request->variable('delay', [0 => 0]);
		$cap = $this->request->variable('cap', [0 => 0]);
		$cooldown = $this->request->variable('cooldown', [0 => 0]);

		$rows = [];

		foreach ($selected as $forum_id => $bot_id)
		{
			$forum_id = (int) $forum_id;
			$bot_id = (int) $bot_id;

			// "Nessun bot" significa nessuna riga: la tabella resta pulita e
			// il listener non deve filtrare binding vuoti a ogni post.
			if ($forum_id <= 0 || $bot_id <= 0)
			{
				continue;
			}

			$topic = !empty($on_topic[$forum_id]);
			$reply = !empty($on_reply[$forum_id]);
			$mention = !empty($on_mention[$forum_id]);

			// Un binding senza nessun innesco non farebbe mai nulla.
			// Attivare almeno la menzione è il comportamento meno sorprendente.
			if (!$topic && !$reply && !$mention)
			{
				$mention = true;
			}
			...
			$rows[] = [
				...
				'delay_seconds'    => max(0, (int) ($delay[$forum_id] ?? 0)),
				'daily_cap'        => max(0, (int) ($cap[$forum_id] ?? 0)),
				'cooldown_seconds' => max(0, (int) ($cooldown[$forum_id] ?? 0)),
				'enabled'          => 1,
			];
		}
