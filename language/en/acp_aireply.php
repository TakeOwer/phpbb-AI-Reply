...
$lang = array_merge($lang, [
	...
	// --- Related discussions search ----------------------------------------
	'AIREPLY_SEARCH_RESULTS'         => 'Related discussions to look up',
	'AIREPLY_SEARCH_RESULTS_EXPLAIN' => 'When somebody posts, the extension searches the board\'s existing index for related discussions and passes the model their <strong>title, section, date and link</strong>. Never the message content: with titles the bot can say "there is a discussion about this, take a look"; with content it would summarise old answers with its usual confidence.<br><br>No new index is built and there is nothing to wait for: it queries the one the administrator already configured. If that backend does not respond, it falls back to a title search that depends on nothing.<br><br><strong>0 disables the feature, and is the default.</strong> Before turning it on use the probe below: this is the only part of the extension that depends on third-party components, and it is worth seeing what it does on your board before it reaches a real reply.',
	'AIREPLY_SEARCH_PROBE'           => 'Test the search',
	'AIREPLY_SEARCH_PROBE_EXPLAIN'   => 'Type a question the way a user would and press the button: you will see which strategy is used, which keywords are extracted and which discussions would be passed to the model. It consumes no tokens and calls no API.',
	'AIREPLY_SEARCH_PROBE_RUN'       => 'Test',
	'AIREPLY_SEARCH_PROBE_STRATEGY'  => 'Strategy used:',
	'AIREPLY_SEARCH_PROBE_KEYWORDS'  => 'Keywords searched:',
	'AIREPLY_SEARCH_PROBE_NOTE'      => 'Note:',
	'AIREPLY_SEARCH_PROBE_EMPTY'     => 'No discussion found. With this question the model would receive nothing extra.',
	'AIREPLY_SEARCH_PROBE_ERROR'     => 'The search raised an error: %s',

	'AIREPLY_SEARCH_STRATEGY_BACKEND' => 'the search engine configured on the board',
	'AIREPLY_SEARCH_STRATEGY_TITLES'  => 'fallback to discussion titles',
	'AIREPLY_SEARCH_STRATEGY_NONE'    => 'no search performed',

	'AIREPLY_SEARCH_NO_KEYWORDS'      => 'no useful words left in the question after removing mentions, punctuation and overly common words',
	'AIREPLY_SEARCH_NO_CONTAINER' => 'the service container is not reachable from this context',
	'AIREPLY_SEARCH_NO_BACKEND'       => 'no search engine configured on the board',
	'AIREPLY_SEARCH_BACKEND_MISSING'  => 'the service "%s" does not exist on this installation',
	'AIREPLY_SEARCH_BACKEND_INCOMPATIBLE' => 'the "%s" backend does not expose the expected methods',
	'AIREPLY_SEARCH_KEYWORDS_REJECTED' => 'the search engine rejected the keywords, most likely because they are too short for its configuration',
	'AIREPLY_SEARCH_BACKEND_FAILED'   => 'the search engine raised an error (%s)',

	'ACL_U_AIREPLY_TRIGGER'      => 'Can receive automated replies from AI bots',

	// --- Instant reply ------------------------------------------------------
	'AIREPLY_INSTANT'         => 'Instant reply',
	'AIREPLY_INSTANT_EXPLAIN' => 'phpBB\'s cron has no timer of its own: it fires when somebody loads a page, and on a quiet section a reply can sit in the queue for a long time even though the model takes a second to produce it.<br><br>With this on, right after a message is posted the board <strong>closes the connection to the browser</strong> and keeps working in the same process: the person who posted already has their page and waits for nothing while the reply is generated. No network request to the board itself, no cookies forwarded, no security setting to lower.<br><br><strong>The delay set in the Forums tab still wins:</strong> if the bot has a 20-second delay there, this option does not shorten it. For a truly instant reply set the delay to 0.<br><br>One message is processed per request, so as not to prolong a process the server still considers active. The rest is left to cron.',
	'AIREPLY_INSTANT_OK'          => 'This server supports the mode (fastcgi_finish_request is available).',
	'AIREPLY_INSTANT_UNAVAILABLE' => '<strong>This server does not support the mode:</strong> the fastcgi_finish_request function is unavailable, so the option has no effect and replies keep arriving through cron. This happens when PHP does not run in FPM mode.',
]);
