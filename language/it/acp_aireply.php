...
$lang = array_merge($lang, [
	...
	'AIREPLY_SEARCH_NO_CONTAINER' => 'il contenitore dei servizi non e\' raggiungibile da questo contesto',
	...
	// --- Risposta immediata ------------------------------------------------
	'AIREPLY_INSTANT'         => 'Risposta immediata',
	'AIREPLY_INSTANT_EXPLAIN' => 'Il cron di phpBB non ha un timer proprio: scatta quando qualcuno carica una pagina, e su una sezione poco frequentata la risposta puo\' restare in coda a lungo anche se il modello impiega un secondo a produrla.<br><br>Con questa opzione attiva, subito dopo l\'invio di un messaggio il forum <strong>chiude la connessione con il browser</strong> e continua a lavorare nello stesso processo: chi ha scritto ha gia\' ricevuto la sua pagina e non aspetta nulla, mentre la risposta viene generata. Nessuna richiesta di rete verso il forum stesso, nessun cookie inoltrato, nessuna impostazione di sicurezza da abbassare.<br><br><strong>Il ritardo configurato nella scheda Forum ha comunque la precedenza:</strong> se in quel forum il bot ha 20 secondi di ritardo, questa opzione non li accorcia. Per una risposta davvero istantanea porta il ritardo a 0.<br><br>Viene elaborato un solo messaggio per richiesta, per non allungare un processo che il server considera ancora attivo. Gli altri restano al cron.',
	'AIREPLY_INSTANT_OK'          => 'Il server supporta questa modalita\' (fastcgi_finish_request disponibile).',
	'AIREPLY_INSTANT_UNAVAILABLE' => '<strong>Il server non supporta questa modalita\':</strong> la funzione fastcgi_finish_request non e\' disponibile, quindi l\'opzione resta senza effetto e le risposte continuano ad arrivare dal cron. Capita quando PHP non gira in modalita\' FPM.',
]);
