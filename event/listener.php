...
class listener implements EventSubscriberInterface
{
	...
	public static function getSubscribedEvents()
	{
		return [
			...
			'core.posting_modify_template_vars'      => 'show_available_bots',
		];
	}
	...
	public function load_language($event)
	{
		...
		/*
		 * Nel pannello di amministrazione si caricano anche i nomi dei moduli.
		 *
		 * phpBB li carica da solo quando disegna il menu, ma non quando mostra
		 * il registro delle attivita': li' traduce soltanto cio' che ha gia' in
		 * memoria, e le voci dell'estensione compaiono come chiavi grezze —
		 * "Modulo aggiunto » ACP_AIREPLY_BOTS" invece del nome leggibile.
		 *
		 * Il carico e' trascurabile (cinque stringhe) e avviene solo nell'ACP:
		 * ADMIN_START e' definito soltanto li'.
		 */
		if (defined('ADMIN_START'))
		{
			$lang_set_ext[] = [
				'ext_name' => 'salvocortesiano/aireply',
				'lang_set' => 'info_acp_aireply',
			];
		}
	...
	/**
	 * Mostra sopra l'editor i bot che si possono chiamare in questo forum.
	 *
	 * Senza questo elenco il multi-bot resta una funzione a metà: con tre
	 * personalità assegnate a un forum, l'utente dovrebbe conoscerne i nomi a
	 * memoria per poterne chiamare una.
	 *
	 * Vengono mostrati solo i bot con l'innesco "Menzioni" attivo. Elencare un
	 * bot che non risponde alle menzioni sarebbe una promessa non mantenuta:
	 * l'utente lo chiamerebbe e non succederebbe nulla.
	 */
	public function show_available_bots($event)
	{
		...
		foreach ($this->bots->get_for_forum($forum_id) as $candidate)
		{
			...
			if (!$binding->on_mention || $bot->username === '')
			{
				continue;
			}

			// Chi sta scrivendo è il bot stesso: non ha senso proporglielo.
			if ($bot->user_id === (int) $this->user->data['user_id'])
			{
				continue;
			}

			$this->template->assign_block_vars('aireply_available_bots', [
				'USERNAME' => $bot->username,
				'LABEL'    => $bot->bot_name !== '' ? $bot->bot_name : $bot->username,
			]);
		...
		if ($count > 0)
		{
			$this->template->assign_vars([
				'S_AIREPLY_BOTS'      => true,
				'L_AIREPLY_BOTS_HINT' => $this->language->lang('AIREPLY_AVAILABLE_BOTS_HINT'),
			]);
		}
	}
	 ...
	 */
	/**
	 * Elabora subito la coda, dopo aver risposto all'utente.
	 *
	 * IL PROBLEMA
	 * Il cron di phpBB non ha un timer proprio: scatta quando qualcuno carica
	 * una pagina. Su una sezione poco frequentata la risposta puo' restare in
	 * coda a lungo, anche se il modello impiega un secondo a produrla.
	 *
	 * PERCHE' NON SI FA COME AI LABS
	 * Quella estensione chiama il forum stesso via HTTP inoltrando i cookie di
	 * sessione, con uno StreamedResponse che scarica subito gli header.
	 * Funziona, ma il suo stesso codice avverte che serve disattivare la
	 * validazione IP di sessione su tutta la board, e le credenziali
	 * dell'utente viaggiano in una richiesta server-a-server.
	 *
	 * COSA SI FA INVECE
	 * Si chiude la connessione con il browser e si continua a lavorare nello
	 * stesso processo. L'utente ha gia' ricevuto la sua pagina e non aspetta
	 * nulla; il job parte subito dopo. Nessuna richiesta di rete verso se'
	 * stessi, nessun cookie inoltrato, nessuna impostazione di sicurezza da
	 * abbassare.
	 *
	 * Dove fastcgi_finish_request() non esiste — SAPI diversi da PHP-FPM — non
	 * si forza nulla: si esce e ci pensa il cron come prima.
	 */
	protected function maybe_run_now(): void
	{
		if (empty($this->config['aireply_instant']))
		{
			return;
		}

		// Senza questa funzione la connessione resterebbe aperta e l'utente
		// vedrebbe la pagina caricare finche' il modello non ha risposto:
		...
		register_shutdown_function(function () {
			/*
			 * Da qui in poi l'utente ha gia' la sua pagina.
			 *
			 * Ogni errore va assorbito: un'eccezione qui non ha piu' modo di
			 * essere mostrata a nessuno, e comunque non deve impedire al cron
			 * di riprovare il job piu' tardi.
			 */
			try
			{
				...
				if (!isset($phpbb_container) || !is_object($phpbb_container))
				{
					return;
				}

				$queue = $phpbb_container->get('salvocortesiano.aireply.job_queue');
				$worker = $phpbb_container->get('salvocortesiano.aireply.job_worker');

				/*
				 * Un solo job per richiesta.
				 *
				 * Elaborarne di piu' allungherebbe un processo che il server
				 * considera ancora attivo, con il rischio di incappare nel
				 * limite di tempo di esecuzione. Gli altri li prende il cron.
				 */
				foreach ($queue->claim(1) as $job)
				{
					...
					{
						$queue->fail($job, 'exception', get_class($e) . ': ' . $e->getMessage(), false);
					}
				}
			}
			catch (\Throwable $e)
			{
				// Silenzio deliberato: nessuno puo' piu' leggere.
			}
		});
	}
	...
	public function enqueue_replies($event)
	{
		...
		/*
		 * Con più bot nello stesso forum, un solo messaggio può innescarne
		 * diversi: tre bot con "Nuovi topic" attivo significano tre chiamate
		 * API e tre risposte in fila a chi si presenta.
		 *
		 * Il tetto vale solo per gli innesco automatici. Una menzione esplicita
		 * non viene mai limitata: se qualcuno chiama tre bot per nome si aspetta
		 * tre risposte, e negargliele sarebbe incomprensibile.
		 */
		$auto_cap = max(0, (int) ($this->config['aireply_max_bots_per_post'] ?? 1));
		$auto_used = 0;
		...
		foreach ($candidates as $candidate)
		{
			...
			if ($trigger !== forum_binding::TRIGGER_MENTION)
			{
				if ($auto_cap > 0 && $auto_used >= $auto_cap)
				{
					continue;
				}
