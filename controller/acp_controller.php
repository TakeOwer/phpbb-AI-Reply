...
use salvocortesiano\aireply\content\topic_search;
...
class acp_controller
{
	...
	/**
	 * Esegue una ricerca di prova e ne descrive l'esito.
	 *
	 * Serve a rispondere alla domanda che conta prima di accendere la funzione:
	 * quale strategia userà questa board, con quali parole chiave e con quali
	 * risultati. Interrogare il backend di ricerca è l'unica parte
	 * dell'estensione che dipende da componenti di terze parti con firme
	 * variabili, e provarla su un caso reale vale più di qualunque promessa.
	 */
	protected function run_search_probe(): string
	{
		$domanda = trim($this->request->variable('search_probe', '', true));
		...
		try
		{
			$risultati = $this->search->find($domanda, (int) $this->user->data['user_id'], 0, 5);
		}
		catch (\Throwable $e)
		{
			return $this->language->lang('AIREPLY_SEARCH_PROBE_ERROR', $e->getMessage());
		}

		$righe = [];

		$strategia = $this->search->get_last_strategy();
		$righe[] = $this->language->lang('AIREPLY_SEARCH_PROBE_STRATEGY')
			. ' ' . $this->language->lang('AIREPLY_SEARCH_STRATEGY_' . strtoupper($strategia));

		$parole = $this->search->get_last_keywords();
		$righe[] = $this->language->lang('AIREPLY_SEARCH_PROBE_KEYWORDS')
			. ' ' . (empty($parole) ? '—' : implode(', ', $parole));

		$nota = $this->search->get_last_note();

		if ($nota !== '')
		{
			$righe[] = $this->language->lang('AIREPLY_SEARCH_PROBE_NOTE') . ' ' . $nota;
		}
		...
		if (empty($risultati))
		{
			$righe[] = $this->language->lang('AIREPLY_SEARCH_PROBE_EMPTY');
		}
		else
		{
			foreach ($risultati as $r)
			{
				$righe[] = '· ' . $r['title'] . '  [' . $r['forum'] . ', ' . $r['date'] . ']';
			}
		}
	...
	public function mode_settings(): void
	{
		...
		if ($this->request->is_set_post('submit'))
		{
			...
			$this->config->set('aireply_instant', $this->request->variable('aireply_instant', 0) ? 1 : 0);
			$this->config->set('aireply_search_results', max(0, min(10, $this->request->variable('aireply_search_results', 0))));
		...
		$this->template->assign_vars([
			...
			'AIREPLY_INSTANT'     => !empty($this->config['aireply_instant']),
			// Senza questa funzione il modo immediato non puo' funzionare:
			// dirlo qui evita di far cercare un guasto che non c'e'.
			'AIREPLY_INSTANT_OK'  => function_exists('fastcgi_finish_request'),
			'AIREPLY_SEARCH_RESULTS' => (int) ($this->config['aireply_search_results'] ?? 0),
			'AIREPLY_SEARCH_PROBE'   => $this->request->variable('search_probe', '', true),
			'AIREPLY_SEARCH_OUTPUT'  => $this->run_search_probe(),
